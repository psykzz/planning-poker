...
/* harmony export */   "ICON_SCORE_MAP": () => (/* binding */ ICON_SCORE_MAP),
/* harmony export */   "SCORE_ICON_MAP": () => (/* binding */ SCORE_ICON_MAP),
/* harmony export */   "deleteScore": () => (/* binding */ deleteScore),
/* harmony export */   "fetchScores": () => (/* binding */ fetchScores),
/* harmony export */   "resetScores": () => (/* binding */ resetScores),
/* harmony export */   "submitScore": () => (/* binding */ submitScore),
/* harmony export */   "updateAllScores": () => (/* binding */ updateAllScores)
/* harmony export */ });
...
const submitScore = async (session, userId, score) => {
  const {
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('scores').upsert([{
    score,
    user_id: userId,
...
const deleteScore = async (session, userId) => {
  const {
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('scores').delete().match({
    session_name: session,
    user_id: userId
  });
  if (error && !Array.isArray(error)) {
...
const resetScores = async session => {
  const {
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('scores').delete().match({
    session_name: session
...
const updateAllScores = async (session, revealed) => {
  const {
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('scores').update({
    revealed
...
const fetchScores = async session => {
  const {
    data: scores,
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('scores').select('user_id,score,revealed').eq('session_name', session);
  if (error && !Array.isArray(error)) {
    throw new Error(JSON.stringify(error));
  }
  return scores;
