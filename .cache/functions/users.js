...
const createUser = async (session, username) => {
  const {
    data: newUser,
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('users').insert([{
    name: username,
...
const updateUserPresence = async (session, userId, last_presence) => {
  if (!userId) return;
  const {
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('users').update({
    last_presence,
    session_name: session
  }).eq('id', userId);
  if (error && !Array.isArray(error)) {
...
const fetchAllUsers = async session => {
  let {
    data: users,
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('users').select('*').eq('session_name', session);
  if (error && !Array.isArray(error)) {
    throw new Error(JSON.stringify(error));
  }
  return users;
...
const fetchUser = async userId => {
  ...
  let {
    data: user,
    error
  } = await _client__WEBPACK_IMPORTED_MODULE_0__.supabase.from('users').select('*').eq('id', userId);
  if (error && !Array.isArray(error)) {
    throw new Error(JSON.stringify(error));
  }
  if (user?.length > 0) {
