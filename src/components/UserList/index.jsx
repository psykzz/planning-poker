...
export const UserList = ({ me, users, scores }) => {
  let lowest = React.useMemo(() => Math.min(...scores.map(score => score.score)), [scores]);
  let highest = React.useMemo(() => Math.max(...scores.map(score => score.score)), [scores]);
  let stddev = React.useMemo(() => standardDeviation(scores.map(score => score.score)), [scores]);
  const scoreByUser = {}
  ...
  return (
    <div className={styles.user_list}>
      ...
      {!!showScores && <div className={styles.averages}>
